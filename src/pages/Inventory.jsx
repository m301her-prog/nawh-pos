...
export default function Inventory() {
  ...
  // استخدام useEffect مع حماية ضد الانهيار
  useEffect(() => {
    let isMounted = true;
    ...
    async function loadProducts() {
      try {
        ...
        // التحقق من أن البيانات مصفوفة قبل التعيين لمنع انهيار الـ map
        let data = [];
        if (Array.isArray(response)) data = response;
        else if (response?.data && Array.isArray(response.data)) data = response.data;
        else if (response?.rows && Array.isArray(response.rows)) data = response.rows;
        
        if (isMounted) setProducts(data);
      } catch (err) {
        console.error("خطأ في جلب المخزون:", err);
        if (isMounted) setProducts([]); // تعيين مصفوفة فارغة في حالة الخطأ
      } finally {
  ...
  const handleSave = async () => {
    try {
      ...
      if (newProduct) {
        // إعادة جلب القائمة بالكامل لضمان تحديث الـ ID والبيانات من السيرفر
        const updated = await neonService.getProducts();
        setProducts(Array.isArray(updated) ? updated : (updated?.data || []));
        setModalOpen(false);
        setForm(EMPTY);
      }
    } catch (err) {
      alert("فشل حفظ المنتج، يرجى المحاولة لاحقاً");
    }
  };
    ...
    setTimeout(() => {
      try {
        ...
        scanner.render(
          ...
          (err) => { /* تجاهل الأخطاء العشوائية أثناء المسح */ }
        );
      } catch (e) {
      ...
      <Card>
        {loading ? (
          ...
          <div className="divide-y divide-gray-100">
            {filtered.map((p) => (
              <div key={p?.id || Math.random()} className="flex justify-between items-center p-4 text-sm">
                <div>
                  <p className="font-bold">{p?.name || 'بدون اسم'}</p>
                  <p className="text-xs text-gray-400">باركود: {p?.barcode || 'غير متوفر'}</p>
                </div>
                <div className="text-left">
                  <p className="font-bold text-teal-700">{formatCurrency(p?.sale_price || 0)}</p>
                  <p className="text-xs text-gray-500">الكمية: {p?.stock_quantity || 0}</p>
                </div>
              </div>
            ))}
          </div>
